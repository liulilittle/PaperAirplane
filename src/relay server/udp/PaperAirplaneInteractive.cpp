...
char* PaperAirplaneInteractive::mmap(int timeout, int* error)
{
	...
	do
	{
		if (self->m_hMap == NULL && (self->m_hMap = OpenFileMappingA(FILE_MAP_ALL_ACCESS, FALSE, PAPERAIRPLANE_CONFIGURATION_MAP)) == NULL) {
			self->m_hMap = OpenFileMappingW(FILE_MAP_ALL_ACCESS, FALSE, (LPWSTR)PAPERAIRPLANE_CONFIGURATION_MAP);
		}
		...
		if (WaitForSingleObject(self->m_hEvent, timeout) != WAIT_OBJECT_0)
		{
			if (error != NULL)
				*error = WAIT_TIMEOUT;
			break;
