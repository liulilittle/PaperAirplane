...
WSABUF* PRXAllocBufferContext(WSABUF* lpBuffers, int dwBufferCount, const struct sockaddr* lpTo)
{
	if (lpBuffers != NULL && dwBufferCount > 0 && lpTo != NULL) {
...
BOOL PRXFreeBufferContext(WSABUF* buf, int dwBufferCount)
{
	if (buf != NULL && dwBufferCount > 0) {
		HANDLE heap = GetProcessHeap();
		for (int i = 0; i < dwBufferCount; i++) {
